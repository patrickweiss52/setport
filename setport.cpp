...
vector <string>msg;
ifstream in("setport.messages_" + lang + ".txt");
string line;
while (!in.eof()){
getline(in, line);
in >> line;
...
if (argTwo.compare("-e") == 0){//checking args[3] == -e
string printenv = "printenv PORT";//string container
...
for(int i = 0; i < argTwo.size(); ++i){//check if each char is a digit
if (!isdigit(argTwo[i])){
error();
usage();
return 1;
...
usingDefaultLang();
outputStream.open("setport.about_en.txt"); //open file into the from stream object
if (outputStream.fail()){
...
void error(){
fstream outputStream;
outputStream.open("setport.error_" + lang + ".txt"); //open file into the from stream object
if (outputStream.fail()){
usingDefaultLang();
outputStream.open("setport.error_en.txt"); //open file into the from stream object
if (outputStream.fail()){
...
fstream outputStream;
outputStream.open(file); //open file into the from stream object
if (outputStream.fail()){
usingDefaultLang();
outputStream.open("setport.usage_en.txt"); //open file into the from stream object
if (outputStream.fail()){
